...
void *updateProgressBarReader(void * ptr);
void *updateProgressBarWriter(void * ptr);
...
int sendFile() {
        ...
        while(!stop) {
                ...
                pthread_t tid;
                if (pthread_create(&tid, NULL, updateProgressBarWriter, NULL) != 0) {
...
int processDataPacket(unsigned char *packet) {
        ...
        if (ctrl == CTRL_DATA) {
                ...
                if ( tmp == 1) {
                        ...
                        free(data);
                        pthread_t tidP;
                        if (pthread_create(&tidP, NULL, updateProgressBarReader, NULL) != 0) {
                                perror("pthread_create()");
                                exit(-1);
                        }
...
void *updateProgressBarReader(void * ptr) {
        ...
        int c = ceil( ((float) file_stat.st_size) / ((float) appProps.fileSize) * PROGRESS_BAR_SIZE);
...
void *updateProgressBarWriter(void * ptr) {
        ...
        int c = ceil( ((float) bytes_read) / ((float) file_stat.st_size) * PROGRESS_BAR_SIZE);
